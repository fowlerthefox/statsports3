...
DBI::dbSendQuery(con, "ALTER TABLE positions ADD CONSTRAINT nodup UNIQUE (player_display_name, time, lat, lon, speed_m_s, heart_rate_bpm, instantaneous_acceleration_impulse, match_day, match_date, team);")
...
read_with_metadata <- function(path, con=con){
  ...
  dbSendQuery(con, "CREATE TABLE IF NOT EXISTS temp_tbl (id SERIAL PRIMARY KEY, 
    player_display_name VARCHAR(64), time VARCHAR(10), time_since_start NUMERIC,
    lat NUMERIC, lon NUMERIC, speed_m_s NUMERIC, heart_rate_bpm NUMERIC, instantaneous_acceleration_impulse NUMERIC,
    match_day VARCHAR(4), match_date VARCHAR(10), team VARCHAR, distance NUMERIC)")
  
  dbWriteTable(con, 'temp_tbl', summarised_df, row.names = FALSE, overwrite=TRUE)
